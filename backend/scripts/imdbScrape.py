...
def main():
	...
	if not dry_run:
		storage.add_columns('movies', to_storage_year, 
					  columns={'imdbId': 'string', 'runtime': 'Int64'})
...
def fetch_imdb(year, error_cutoff):
	...
	for movId in movie_data.index:
		try:
			...
			title_correct = ('Title' in response_dict) and (response_dict['Title'] == title)
			year_correct = ('Year' in response_dict) and (response_dict['Year'] == str(year))
			awards_correct = ('Awards' in response_dict) and (re.match(r'^Nominated for \d+ Oscar', response_dict['Awards'])!=None 
						or re.match(r'^Won \d+ Oscar', response_dict['Awards'])!=None)
			if sum([not title_correct, not year_correct, not awards_correct]) >= error_cutoff:
				print(f"Problems with {title}.")
				if 'Title' not in response_dict:
					print("No title found.")
				elif not title_correct:
					print(f"Title mismatch: my <{title}> vs their <{response_dict['Title']}>")
				if 'Year' not in response_dict:
					print("No year found.")
				elif not year_correct:
					print(f"Year mismatch: my <{year}> vs their <{response_dict['Year']}>")
			...
			if 'Runtime' in response_dict:
				new_data['runtime'] = int(re.match(r'\d*',response_dict['Runtime']).group())
			else:
				print(f"Missing data for {title}")
			if not new_data:
				continue
			if not dry_run:
				storage.update_movie(movId, to_storage_year, 
							new_data=new_data)
			if dry_run:
