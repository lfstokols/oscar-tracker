...
sys.path.append(str(BACKEND_DIR)) # Adds the parent directory to the path for module imports
									# Should be backend/
									# N.B. first .parent goes from file to directory, second goes to parent directory
sys.path.append(str(LOGIC_DIR)) # Adds the logic directory to the path for module imports
...
def main():
	parser = argparse.ArgumentParser(description='Attempts to parse nomination data from Wikipedia.')
	parser.add_argument('year', type=int, 
					 help='Year to import data for. Year is when movies are released, not when the Oscars ceremony is held.')
	parser.add_argument('--dry-run', action='store_true', help='Saves data in a test folder.')
	parser.add_argument('--overwrite', action='store_true', help='Deletes existing data before adding new entries.')
...
def fetch_wikipedia(year):
	...
	ordinal_name = f"{ceremony_num}th" if 11 <= ceremony_num % 100 <= 13 else str(ceremony_num) + {1: 'st', 2: 'nd', 3: 'rd'}.get(ceremony_num % 10, 'th')
	...
	is_current = datetime.now() < datetime(year, 4, 1) #Check if the Oscars ceremony has happened yet. Not super accurate, but good enough for our purposes.
	debug_print("Year is current" if is_current else "Year is in the past")
	section_id = "Nominations" if is_current else "Awards" # The section of the page where the data is located
...
def parse_data(data_list):
	...
	debug_print("Extra entries are: " + str(set(list(nominations_raw.keys())) - set(category_list)))
	...
	for category in category_list:
		for line in nominations_raw[category]:
			if category == 'cat_song':
				...
				group_num = 2
			elif category_df.loc[category, 'grouping'] == 'Acting': #in ["Best Actor", "Best Actress", "Best Supporting Actor", "Best Supporting Actress"]:
			...
			if category_df.loc[category, 'hasNote']:
				try:
					note = re.match(pattern, line).group(3 - group_num).strip() # 3 - group_num is 2 if group_num is 1, and vice versa
					assert not too_many_dashes
				except Exception as e:
					print(f">\tUnable to parse note for {category}::{line}")
					print(f">\tBest guess is <{note}>. Leave blank to use guess.")
					note = input("\t>>Please enter the NOTE manually: ").strip() or title
...
def get_movie_id(title: str) -> MovID: # Returns the id if it exists, otherwise creates one and adds to table
