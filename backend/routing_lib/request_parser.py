...
from backend.routing_lib.error_handling import YearError, APIArgumentError
...
def get_active_user_id(request: Request) -> UserID:
    ...
    if code != 0:
        logging.error(
            f"Got a request to {request.endpoint}[{request.method}] that needs an active user, but value {active_user_id} is invalid"
        )
        raise APIArgumentError("Invalid active user id", [("activeUserId", "cookie")])
    users = qu.get_users()
    if id not in [user["id"] for user in users]:
        logging.error(
            f"Got a request to {request.endpoint}[{request.method}] with active user {active_user_id}, which IS valid per se, but is NOT in my database"
        )
