...
async def get_justwatch(tmdb_id: int) -> tuple[str, int]:
    """
    ...
    if ott_div is None or type(ott_div) != Tag:
        raise externalAPIError(
            f"No JustWatch URL found for {tmdb_id}: no ott_title block found")
    text = ott_div.find_next('p')
    if text is None or type(text) != Tag:
        raise externalAPIError(
            f"No JustWatch URL found for {tmdb_id}: no text found after ott_title block")
    ...
    if link is None or type(link) != Tag or "href" not in link.attrs:
        ...
        raise externalAPIError(
            f"No JustWatch URL found for {tmdb_id}: no link found after ott_title block")
    ...
    if justwatch_url is None or type(justwatch_url) != str:
        raise externalAPIError(
            f"No JustWatch URL found for {tmdb_id}: link href is not a string")
...
async def get_Imdb(movie_db_id: int) -> str:
    """
    Get the IMDB URL for a given TMDB ID.
    """
    url = "https://api.themoviedb.org/3/"
    headers = {"Authorization": f"Bearer {movie_db_key}",
    ...
    if "imdb_id" not in data:
        raise externalAPIError(
            f"No IMDB URL found for {movie_db_id}: no imdb_id found in external_ids")
