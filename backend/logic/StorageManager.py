...
class StorageManager:
	def __init__(self, database_directory):
		...
		self.max_retries = 10
		# Flavor info
		self.flavor_list: dict[DataFlavor] = ['movies', 'users', 'nominations', 'categories', 'watchlist']
		self.flavor_aliases = {**{flavor: flavor for flavor in self.flavor_list}, # Each flavor is its own alias
						 **{flavor[:1]: flavor for flavor in self.flavor_list}, # First letter of each flavor is an alias
			'mov': 'movies', 'usr': 'users', 'cat': 'categories', # aliases from id prefixes
			}
		...
		self.DEFAULT_WATCHLIST.astype({'userId': 'string', 'movieId': 'string', 'status': 'string'})
	...
	def create_unique_id(self, flavor: DataFlavor, existing_ids) -> MovID|UserID: #type is a string, 'movie' or 'user'
		flavor = self.format_flavor(flavor)
		assert self.flavor_props(flavor)['shape'] == 'entity', f"Flavor '{flavor}' doesn't have IDs."
		assert flavor != 'categories', "Category IDs are static. You can't create new ones."
		try:
		...
		tries = 0
		while tries < 100:
			id = id_prefix + f'{random.randint(0, 0xFFFFFF):06x}'
	...
	def flavor_props(self, flavor_indic, is_filename=False) -> dict[Literal['shape', 'static', 'annual'], str|bool]:
	...
	def add_columns(self, flavor: DataFlavor, year: str | int | None = None, columns: dict = {}):
		def operation(data):
	...
	def update_movie(self, movie, year, new_data: dict = {}, try_title_lookup=False) -> MovID|bool:
		for val in new_data.values():
			if ',' in str(val):
				print(f"There's a comma in the data. That will probably mess up the CSV file.\n"
		...
		if not try_title_lookup:
			...
			def operation(data):
				if movie in data['title'].tolist():
					...
					movie_id = self.create_unique_id('m', year)
	...
	def add_user(self, username, letterboxd = None, email = None):
		...
		def operation(data: pd.DataFrame):
			user_id = self.create_unique_id('users', userIdList)
			#while True:
	...
	def add_watchlist_entry(self, year, userId, movieId, status: WatchStatus) -> bool:
		...
		assert status in ['seen', 'todo'], f"Invalid status '{status}'."

		def operation(data: pd.DataFrame):
			existing_entry = data[(data['userId'] == userId) & (data['movieId'] == movieId)]
			data = data.drop(existing_entry.index)
			new_entry = pd.DataFrame({'userId': userId, 'movieId': movieId, 'status': status})
			data = data.concat([data, new_entry], ignore_index=True)
			return data, bool(existing_entry)
