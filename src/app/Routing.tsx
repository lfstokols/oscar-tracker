...
export default function Routing() {
  return (
    <Routes>
      <Route
        element={<Navigate to={`/${LEGACY_URL}/${DEFAULT_YEAR}`} />}
        path="/"
      />
      <Route element={<App />}>
        ...
        <Route
          element={<Navigate to={`/${BY_USER_URL}/${DEFAULT_YEAR}`} />}
          path={`/${BY_USER_URL}`}
        />
        <Route element={<CategoryTab />} path={`/${BY_CATEGORY_URL}/:year`} />
        <Route
          element={<Navigate to={`/${BY_CATEGORY_URL}/${DEFAULT_YEAR}`} />}
          path={`/${BY_CATEGORY_URL}`}
        />
      </Route>
    </Routes>
  );
